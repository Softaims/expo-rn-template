  ...
  return (
    <RangeSlider
      ...
      onValueChanged={(newLow, newHigh) => {
        setLow(newLow);
        setHigh(newHigh);
        console.log(`Price: $${newLow} - $${newHigh}`);
      }}
    />
  );
};
...
const AgeRangeExample = () => {
  ...
  return (
    <RangeSlider
      ...
      renderRail={Rail}
      renderRailSelected={RailSelected}
      renderLowValue={(value) => <Text style={styles.valueText}>{value} yrs</Text>}
      renderHighValue={(value) => (
        <Text style={styles.valueText}>{value === 65 ? `${value}+` : `${value} yrs`}</Text>
      )}
      onValueChanged={(newLow, newHigh) => {
        setLow(newLow);
        setHigh(newHigh);
        console.log(`Age Range: ${newLow} - ${newHigh}`);
      }}
    />
  );
};
...
export const SingleControl: Story = {
  render: () => <SingleControlExample />,
};

const DisabledRangeExample = () => {
  const [low] = useState(30);
  const [high] = useState(70);
  ...
  return (
    <RangeSlider
      ...
      renderHighValue={HighValue}
      onValueChanged={(newLow, newHigh) => {
        console.log(`Disabled: ${newLow} - ${newHigh}`);
      }}
    />
  );
};
...
export const DisabledRange: Story = {
  render: () => <DisabledRangeExample />,
};

const SmallStepsExample = () => {
  const [low, setLow] = useState(2.5);
  const [high, setHigh] = useState(7.5);
